...
document.querySelector("#empleadosMas300").addEventListener("click", (e) => {
    ...
    try {
        ...
        let mas=document.querySelector(".main-container").classList.toggle("nv");
        cargarArrayDesdeHTML(); 
        // let m=(mas.classList.replace(nv,true))?"operar":"salir";
        if(!mas){
           e.target.innerText="salir"
        }else{e.target.innerText="sueldos d"}
        // e.target.value
