...
valdarCampos = () => {
    ...
    if (sueldos.value.length > 10) { 
        declaracion = formNul(sueldos, "menos de 20 caracteres")
    }
       if (nombres.value.length > 10) { 
        declaracion = formNul(nombres, "menos de 20 caracteres")
    }
    if (!nombres.value.length < 3) {
        declaracion = formNul(nombres, "mas de 3 caracteres")
    }
    // if (nombre.test(".")) {alert("NO")}else{     error=1}
    // if (!/[a-zA-Z0-9]{4,16}/.test(nombres.value)) {
    //     console.log("es:" + !/\w/.test(nombres.value));
...
forms.addEventListener("submit", (e) => {
    remover.classList.remove("borde")
    // forms.classList.add("f")
    e.preventDefault();
    
     verifica = valdarCampos();
