...
valdarCampos=()=>{
    ...
    if (nombre) {formNul(input,"imcompleto")}  else{formOk(nombre)}
    if (sueldo) { formNul(input,"imcompleto2")} else{if (isNaN(sueldo)){formNul(input,"no es numero")}}
...
formOk = (unDato) => {
    ...
    // const teta=/\w/
        ...
        if (unDato.lenght > 20) {alert("NO")}else{     errors=1}
        if (unDato.lenght < 3) {alert("NO")}else{     errors=1}
        // if (nombre.test(".")) {alert("NO")}else{     error=1}
        if (/\w/.test(unDato)) {alert("NO")}else{     errors=1}
...
formNul=(input,n)=>{
    const suPadre=input.parentElement;
    let nota= suPadre.document.querySelector(".nota")
    // const errores=0
    nota.innerHTML=n
