...
valdarCampos=()=>{
    ...
    if (!nombre.value)
    {alert("NO21")
        formNul(nombre.parentElement,"imcompleto")} 
     else{formOk(nombre)}
    // if (!sueldo.value.trim) { formNul(sueldo,"imcompleto2")} else{if (isNaN(sueldo)){formNul(sueldo,"no es numero")}}
...
formOk = (unDato) => {
    ...
    // const teta=/\w/
        ...
        if (unDato.lenght > 20) {alert("NO1")}else{     errors=1}
        if (unDato.lenght < 3) {alert("NO2")}else{     errors=1}
        // if (nombre.test(".")) {alert("NO")}else{     error=1}
        if (/\w/.test(unDato)) {alert("NO3")}else{     errors=1}
...
formNul=(inputs,error)=>{
    // const suPadre=inputs.
    const donde= inputs.querySelector("p")
    // const errores=0
    donde.innerText=errror
