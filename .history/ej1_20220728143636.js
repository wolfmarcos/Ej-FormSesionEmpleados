...
formOk = (inputs) => {
    ...
    // const teta=/\w/
        ...
        if (inputs.lenght > 20) {alert("NO1")}else{   alert("NO11")  }
        if (inputs.lenght < 3) {alert("NO2")}else{  alert("NO22")  }
        // if (nombre.test(".")) {alert("NO")}else{     error=1}
        if (/\w/.test(inputs)) {formNul(inputs,"llene los campos")}else{     errors=1}
